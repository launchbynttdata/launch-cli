...
def test_terragrunt_plan_run_all(mock_run):
    ...
    terragrunt_plan(file=None, run_all=True, dry_run=False)
...
def test_terragrunt_plan_no_run_all(mock_run):
    mock_run.return_value = MagicMock()
    terragrunt_plan(file=None, run_all=False, dry_run=False)
...
def test_terragrunt_plan_with_file(mock_run):
    mock_run.return_value = MagicMock()
    terragrunt_plan(file="plan.out", run_all=False, dry_run=False)
